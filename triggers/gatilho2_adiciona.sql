...
CREATE TRIGGER IF NOT EXISTS calcularDataDeChegadaUpdate

BEFORE UPDATE 
ON Viagem FOR EACH ROW
BEGIN 
    UPDATE Viagem
    SET dataDeChegada = datetime(strftime('%s',new.dataDePartida) + 60 * (SELECT max(tempoDeChegada) FROM Rota JOIN Informacao USING(idRota) WHERE idRota = new.idRota),'unixepoch')
    WHERE idViagem = new.idViagem;
END;
